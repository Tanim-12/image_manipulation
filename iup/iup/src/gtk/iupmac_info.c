...
#else
  CGRect __bounds = CGDisplayBounds(mainDisplayID); rect.left = (short)__bounds.origin.x; rect.top = (short)__bounds.origin.y; rect.right = (short)(__bounds.origin.x + __bounds.size.width); rect.bottom = (short)(__bounds.origin.y + __bounds.size.height);
#endif
